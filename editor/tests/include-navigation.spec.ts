...
function subheadingRuleLine(): number {
    const text = fs.readFileSync(path.join(__dirname, '../../examples/regulatory/customs/gri.le'), 'utf8');
...
test.describe('Navigation into included resources', () => {
    test('clicking a node proved in an included file opens that file at the rule, in a tab', async ({ page }) => {
        ...
        await page.goto('index.html?example=regulatory/customs/apparel_cbp&scenario=ny_n362700&query=subheading');
        ...
        expect(new URL(page.url()).searchParams.get('example')).toBe('regulatory/customs/apparel_cbp');
        ...
        expect(new URL(page.url()).searchParams.get('example')).toBe('regulatory/customs/gri');
