...
function detectTargetLanguage(text) {
  ...
  for (const info of languages) {
    if (info.opener && norm.startsWith(info.opener.toLowerCase())) {
      const m = firstStatement.match(/:\s*(\w+)/);
...
// src/share-url.ts
var PARAM = "lzp";
async function streamBytes(input, transform) {
...
async function compressToParam(text) {
...
async function decompressFromParam(value) {
  const bytes = await streamBytes(
    fromBase64Url(value),
...
function lineSpan(text, lines) {
  ...
  return end > start2 ? [start2, end] : null;
}
    ...
    if (res && res.results && res.results.length > 0) {
      ...
      res.results.forEach((result, index) => {
        ...
        if (result.modelCount && result.modelCount > 1 && result.modelIndex) {
          ...
          const assumed = Array.isArray(result.assumptions) ? result.assumptions : [];
          if (assumed.length > 0) {
      ...
      answersList.textContent = t("The query did not finish within {n} seconds and was stopped: a rule may loop, or the search may be too large.").replace("{n}", String(res.timeLimit));
      this.setStrongestReason();
...
function docQueryAt(model, position, selection, languageId) {
...
`
  ...
  function reportExampleLoadError(data) {
...
${err}`, "See s(CASP)");
  ...
  const isLeFile = (name) => /\.le$/i.test(name);
  const isBinaryUpload = (name) => /\.(zip|xlsx|docx)$/i.test(name);
  const toBase64 = (buf) => {
  ...
  const importForeignFile = async (file) => {
    ...
    try {
      ...
      const lines = [
        ...
        todos ? `${todos} ${t("fragment(s) could not be translated: they are in the program as TODO comments.")}` : t("Everything was translated."),
        ...(data.notes || []).slice(0, 12).map((n) => "\u2022 " + n)
      ];
  ...
  const showImportReport = (lines) => {
    ...
    box.style.cssText = "position:fixed;top:70px;right:16px;max-width:520px;z-index:3000;background:var(--menu-bg,#eee);color:inherit;border:1px solid #888;border-radius:6px;padding:10px 30px 10px 12px;font-size:12px;box-shadow:0 2px 8px rgba(0,0,0,.3);white-space:pre-wrap";
  ...
  const openPickedFile = async (file, handle) => {
  ...
  const pickAndOpen = async (onlyForeign) => {
    ...
    if ("showOpenFilePicker" in window) {
      try {
        ...
        for (const f of formats) {
          types.push({
            description: `${f.title} \u2014 ${t("translated on opening")}`,
  ...
  const largerReasonsInput = document.getElementById("larger-important-reasons-input");
  ...
  const LPS_API = new URLSearchParams(window.location.search).get("lpsapi") ?? localStorage.getItem("lps-lpsapi") ?? "http://localhost:3060/lpsapi";
  async function legalViewWithRun(doc) {
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), 3e4);
    try {
      const token = localStorage.getItem("lps-token") || "";
      const body = {
        operation: "le_legal_view",
  ...
  async function openLegalView(doc) {
    ...
    if (withRun) {
      await openDocument(withRun, { fileName: `${stem}_legal_view.le` });
      return;
    }
    try {
      const response = await fetch("/leapi", {
  ...
  async function openTestReport(doc) {
    document.getElementById("test-report")?.remove();
    const overlay = document.createElement("div");
    overlay.id = "test-report";
    overlay.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:3000;display:flex;align-items:center;justify-content:center";
    const box = document.createElement("div");
    box.style.cssText = "background:var(--panel-bg,#252526);color:var(--text-color,#d4d4d4);border:1px solid #555;border-radius:8px;width:min(1100px,95vw);max-height:88vh;display:flex;flex-direction:column;padding:14px 16px;font-size:13px";
    overlay.appendChild(box);
    const head = document.createElement("div");
    ...
    cb.type = "checkbox";
    onlyFailures.appendChild(cb);
    onlyFailures.appendChild(document.createTextNode(" " + t("only those that did not pass")));
    ...
    if (!data || data.error) {
      summary.textContent = data?.error || t("no answer from the server");
      return;
    }
    const tests = data.tests || [];
    summary.textContent = tests.length ? `${data.passed} ${t("passed")}, ${data.failed} ${t("failed")}${data.errors ? `, ${data.errors} ${t("errors")}` : ""} \u2014 ${tests.length} ${t("tests")}` : t('This program has no tests: a scenario states them with "<query> expects answers [...]".');
    const render = () => {
      scroller.innerHTML = "";
      ...
      for (const r of tests) {
        ...
        cells.forEach((c, i) => {
          ...
          td.style.cssText = "border-bottom:1px solid #444;padding:4px 6px;vertical-align:top" + (i === 0 ? `;color:${r.status === "pass" ? "#3fb950" : "#f85149"};font-weight:bold` : "");
          tr.appendChild(td);
        });
