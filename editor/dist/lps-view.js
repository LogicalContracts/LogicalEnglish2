...
var LE_BASE = params.get("leapi") ?? localStorage.getItem("lps-leapi") ?? "/leapi";
var LPS_BASE = params.get("lpsapi") ?? localStorage.getItem("lps-lpsapi") ?? "http://localhost:3060/lpsapi";
var LE_TOKEN = params.get("letoken") ?? "myToken123";
var LPS_TOKEN = params.get("token") ?? localStorage.getItem("lps-token") ?? "";
...
async function compileAndRun() {
  ...
  try {
    ...
    if (state.mode === "le") {
      ...
      if ((le.issues ?? []).some((i) => i.severity === "error")) {
...
function addMarkers(items) {
  const model = state.editor.getModel();
  const existing = monaco.editor.getModelMarkers({ resource: model.uri }).filter((m) => m.owner === "lps");
...
async function renderTimeline() {
  ...
  if (!state.session) {
    pane.replaceChildren(el("p", { class: "empty", text: "Run a program first." }));
    return;
  }
...
var activePane = () => document.querySelector(".tabs button.on").dataset.pane;
var renderActivePane = () => PANES[activePane()]();
var SAMPLE_LE = `the target language is: lps.
