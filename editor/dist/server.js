...
function tokenize(text) {
  ...
  while (i < text.length) {
    ...
    if (lineStart) {
      ...
      if (indent > 0) {
        tokens.push({ type: 0 /* Indent */, value: indent, start: start2, end: i });
      }
      lineStart = false;
      continue;
    }
    ...
    if ([">=", "<=", "=<", "==", "!="].includes(multiPunct)) {
      tokens.push({ type: 3 /* Punctuation */, value: multiPunct, start, end: i + 2 });
      i += 2;
      continue;
    }
    if (text.startsWith("/*", i)) {
      ...
      if (endIdx !== -1) {
        ...
        tokens.push({ type: 6 /* Comment */, value: content, start, end: endIdx + 2 });
        i = endIdx + 2;
      } else {
        tokens.push({ type: 6 /* Comment */, value: text.substring(i + 2), start, end: text.length });
        i = text.length;
      }
      continue;
    }
    if (char === "%") {
      ...
      tokens.push({ type: 6 /* Comment */, value: content, start, end: actualEnd });
      i = actualEnd;
      continue;
    }
    const dateMatch = text.substring(i).match(/^\d{4}-\d{2}-\d{2}/);
    if (dateMatch) {
      tokens.push({ type: 4 /* Date */, value: dateMatch[0], start, end: i + dateMatch[0].length });
    ...
    if (char === '"' || char === "'") {
      ...
      if (endIdx !== -1) {
        ...
        tokens.push({ type: 5 /* String */, value: text.substring(i + 1), start, end: text.length });
        i = text.length;
      }
      continue;
    }
    ...
    if (/[!@#$%^&*()\-+={}\[\]:;"'<>,.?\/|\\~]/.test(char)) {
      tokens.push({ type: 3 /* Punctuation */, value: char, start, end: i + 1 });
      i++;
      continue;
    }
    i++;
      ...
      if (lastHeaderLine !== -1) {
        ...
        if (endLine > lastHeaderLine) {
          foldingRanges.push({
            ...
            endLine,
            kind: import_browser.FoldingRangeKind.Region
          });
        }
      }
      lastHeaderLine = i;
...
async function validateTextDocument(textDocument) {
  ...
  const diagnostics = [];
  for (const token of tokens) {
    if (token.type === 5 /* String */ && !text.substring(token.start, token.end).endsWith(text[token.start])) {
      diagnostics.push({
        severity: import_browser.DiagnosticSeverity.Error,
        range: {
    ...
    if (!templateMatch) {
      switch (token.type) {
        case 1 /* Word */:
          ...
          const keywords = ["includes", "if", "and", "or", "either", "all of", "which", "sum", "count", "average", "min", "max", "is", "are", "a", "an", "the", "such that"];
