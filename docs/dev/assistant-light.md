...
*Kind: design, as built · Audience: developers · Status: implemented (2026-06); some sections are still the proposal*

> **Status: Implemented.**
> This document describes the lighter implementation of the
> [LE Assistant](assistant.md). It shares the same chat UI and the
...
The Deep agent reads files from disk on demand (`read`, `glob`, `docs/user/reference/language.md`,
...
2. **LE syntax** — the full text of `docs/user/reference/language.md` (~10 KB; small enough
   to inline verbatim).
...
1. **Add a structured, stable header** (YAML frontmatter or a fenced
   ...
   ```yaml
   ---
   le_syntax_doc: docs/user/reference/language.md
...
Identical to the existing endpoints (see [`docs/user/api/web-api.md`](../user/api/web-api.md)) with one new
field:
...
*See also: [`docs/dev/assistant.md`](assistant.md) for the Deep mode,
[`docs/user/api/web-api.md`](../user/api/web-api.md) for the `/leapi` wire protocol, and
[`docs/user/reference/language.md`](../user/reference/language.md) for the LE syntax the Light prompt inlines.*
