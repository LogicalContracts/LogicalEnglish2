...
# Bugs from Jacinto, etc.

## Comment colouring
In some large comments sometimes the editor is colouring some of the text and it shouldn't; for example examples/moreExamples/collections/kowalski-book/underground_emergency.le, starting at line 9

## Example aliases/synonyms
The example tree now uses aliases, for example examples/moreExamples/collections/kowalski-book/underground_emergency can be edited via URL /editor/index.html?example=rkBook/underground_emergency ; where are these synonyms defined? Shouldn't this be in the documentation somewhere?

## Proof Game
The Proof Game refuses to open for a failed query ("You need a query with an answer to play"). however it is able to deal with NAF... couldn't we allow the game to play for a failed top node too?

## Improving generation of displays
The 2D and 3D animations generated by the assistant seem still very basic and erractic, as indicated in the display problems below. Maybe the prompts and generation process could be improved. Some wild thoughts, but you please do some research too:
-   might our Automaton states (the finite automata perspective on the program) provide hints about relevance or focus ?
-   Might an Interactive Fiction perspective help? a 2d/3d rendering of an execution tells its "narrative" graphically. Can one detect high level patterns either in the source - composite events, intensional fluents and their dependencies - or in the execution trace (in addition to the basic geometry ones like stack) that guide the narrative building hence the scene construction? Perhaps use multiple scenes for important program states...?


Now the two reported problems, that occurred for /lps2/examples/collections/kowalski-book/underground.lps :

### Display problem 1
an animation I generated (sorry I lost its  display facts by now) showed a fire on top of a box to signal the emergency fire. It would be interesting seeing the other fluents too:  alerted, in_station, stopped.

### Display problem 2
In another run the assistant generated these facts:

display(emergency(Value), [type:rectangle, from:[0.00, 0.00], to:[150.00, 40.00],
		     fillColor:'#2f3542', label:(Emergency:Value)]).

display(penalty(Value), [type:rectangle, from:[162.00, 0.00], to:[312.00, 40.00],
		     fillColor:'#2f3542', label:(Penalty:Value)]).


display(timeless, [
	[type:rectangle, from:[-14, -26], to:[326.00, 86.00], strokeColor:'#2a2f3a'],
	[type:text, point:[0.00, 44.00], content:'Emergency', fontSize:11, fillColor:'#8b94a6'],
	[type:text, point:[162.00, 44.00], content:'Penalty', fontSize:11, fillColor:'#8b94a6'],
	[type:text, point:[0, 66.00], content:'London Underground Emergency Notice', fontSize:15, fillColor:'#dfe3ea']
	]).    
...
ISSUE
lps_no_such_session
Unknown error term: lps_no_such_session('s1-2553d9')
ID: faed16fd2be049b48af88399c59723b4
Sept. 20, 2026, 7:04:34 a.m. UTC
Project	lps2
environment	production
Level	error
Exception

lps_no_such_session: Unknown error term: lps_no_such_session('s1-2553d9')
User
...
# Bugs reported by Jacinto
Please figure out the examples from their URLs below.
## highlighting
In https://le2.logicalcontracts.com/editor/index.html?example=enclosure&scenario=test&query=three&answer=1 , the highlighting of 
        and it is not the case that
in line 33 is wrong. "and" is correctly in purple. But "it is not the case" must also be purple. Not only the "that" at the end (which is correct).

## discrepancy
In https://le2.logicalcontracts.com/editor/index.html?example=migration%2Fscasp%2Fobligation%2Fobligation&scenario=test&engine=scasp&query=query_1&answer=1

There is a discrepancy between the answer:

the borrower cures the failure of any obligation on 2016-06-02 that any requirement

and the explanation:

the borrower cures the failure of an obligation on 2016-06-02 that the borrower pays 525 to the lender on a date
  the borrower pays 525 to the lender on 2016-06-02

The explanation is correct. 

## wrong answer
In https://le2.logicalcontracts.com/editor/index.html?example=migration%2Fscasp%2Fsubset%2Fsubset&scenario=two&query=query_1&engine=scasp&answer=2

It is producing this wrong answer:

[Alice Bob] is a subset of [Alice]
...
[Alice Bob] is a subset of [Alice]
  [Alice Bob] is a set
  [Alice] is a set
  x le_forall_1(['Alice','Bob'],['Alice'])
    x a thing belongs to [Alice Bob]
      x an any is in [Alice Bob]

Both are wrong

## Questionable comment in program example
In https://le2.logicalcontracts.com/editor/index.html?example=abduction%2Fdiagnosis&scenario=checkup&query=diagnose&answer=4

..its commentaries say this:

% Logical English has no integrity constraints, but the effect of slide 28's
% passive constraint ("it is not the case that the sprinkler was on if the
% sprinkler is broken") is obtained here with negation as failure over closed
% (scenario) facts: 

Don't we have integrity constraints in pure LE already...? If so, just edit the comment, the example is working fine

## Noise in explanation
In https://le2.logicalcontracts.com/editor/index.html?example=happy_dragon&scenario=smoky&query=happy&engine=scasp&answer=2

Scasp is working well in this example, but the explanation contains that le_forall/1 reference

alice is happy
  alice is a dragon
  x le_forall_1(alice)
    x alice is a parent of a dragon
    alice is a parent of bob
    bob is healthy
      bob is a dragon
      x bob smokes
        x a creature is a parent of bob
        alice is a parent of bob
        alice is a dragon
        x alice smokes
          x a creature is a parent of alice
          x alice is a parent of alice

# Making of
Users have expressed concerns about the authorship of examples in LE and LPS, specifically whether they were invented by an agent (you or a predecessor of yours), or generated by a deterministic converter from some twin original, or adapted from a book by you, etc. So I want you to please go over every example folder in the LPS2 and LE2 systems, and, if the folder contains files not authored by a humann, add a MAKING_OF.md file summarising how the examples were built. This file will not be visible in the IDEs, it will simply be stored in our repositories.
