...
*Kind: reference · Audience: developers · Status: preliminary draft, covers part of the /leapi operations*
...
| `source` | string | (Optional) the example the document came from, e.g. `"language/abduction/loan_approval"`, so its relative include resources resolve against that example's folder — as for `load` |
...
| `whyNot` | boolean | (Optional) for a query with no answer, also reply `unmet`: the conditions it did not meet (docs/user/reference/language.md §17.10, "Why not") |
