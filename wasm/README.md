...
| `pack.pl` | what goes into the payload, and (more to the point) what must not: the rule is `restricted_paths.pl`'s own, for a visitor with no roles. It also carries `docs/user/**.md`, which is what the Light Assistant searches and cites, and leaves out the large public trees of `light_excluded/1` (customs, Medicare, OIPA twins): the browser build is "LE light" |
