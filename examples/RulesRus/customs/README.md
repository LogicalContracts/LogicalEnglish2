...
```
% the same in Chapter 62 note 4 for headings 6205 and 6206
...
has a collar, confer "zips through a self-fabric stand-up collar"`. The documents say where they are
...
provenance and say so in a comment. The Explanatory Notes are not public; rules that encode them
...
- **Only what the ruling states is a fact.** Each style is described by the
  ...
  material, the components, the form, the polymer — each quoting its passage
  (`confer "..."`) under the scenario's default source (`scenario ny_n362700
  is, as stated in ruling NY N362700:`). **The ruling's code is never a fact.**
- **What the office decided is a judgment** (`; judged`), recorded with who
  ...
  outerwear is rejected, according to CBP, confer "the submitted sample lacks
  the character of an outerwear jacket"`), with a `because "..."` where the
  office's reasoning is not in the quoted words. Laboratory findings and
  importers' statements carry their source the same way.
...
- **How the scenarios were written.** The facts of the scenario files were
  first transcribed by hand from each ruling, and the passage quoted for each
  fact picked from the style's own description by a one-off script (not kept:
  it was tied to these texts' wording). The `.le` files are now the source:
  edit them directly, and the verifier checks every quotation against
  `sources/` (a changed or mistyped passage is reported). New rulings come in
  through *Write it in English… → From a document* (below).
...
the subheading of the Pom Pom Launcher is 3924.90
  the heading of the Pom Pom Launcher is 3924
    the Pom Pom Launcher is clear of rival headings
      the Pom Pom Launcher is claimed to belong to toys of heading 9503,
