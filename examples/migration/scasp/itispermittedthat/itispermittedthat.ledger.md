...
| and_are_at_most_days_apart/3 | predicate | encoded | a template (the #pred wording, else a naive one) | *a date* and are at most days apart *a time* with *a thing* |  |
...
| designates_at_that/3 | predicate | encoded | a template (the #pred wording, else a naive one) | *a party* designates at *a time* that *an eventuality* |  |
...
| ?- 'It_is_permitted_that'(designates_at_that(_87134,_87136,occurs_at(_87142,_87144))) | query | encoded | a ?- query -> a query, asked in every scenario | the queries |  |
