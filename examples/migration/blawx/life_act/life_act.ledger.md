...
| dob/2 | category, attribute or relationship | encoded | its #pred wording -> a template | *a person* was born on *a time* |  |
| dod/2 | category, attribute or relationship | encoded | its #pred wording -> a template | *a person* died on *a time* |  |
...
| blawxtest when_did_bob_live | test | residue | a Blawx test -> a scenario; its expected answers are Blawx's own (oracle.py) | when_did_bob_live | the query blawx_during(_24422,alive(bob),_24426) is not translated:  |
| blawxtest when_is_bob_not_alive | test | residue | a Blawx test -> a scenario; its expected answers are Blawx's own (oracle.py) | when_is_bob_not_alive | the query blawx_during(_24282,-alive(bob),_24286) is not translated:  |
...
- **blawxtest when_did_bob_live** (test) — a Blawx test -> a scenario; its expected answers are Blawx's own (oracle.py); in the program: when_did_bob_live. the query blawx_during(_24422,alive(bob),_24426) is not translated: 
- **blawxtest when_is_bob_not_alive** (test) — a Blawx test -> a scenario; its expected answers are Blawx's own (oracle.py); in the program: when_is_bob_not_alive. the query blawx_during(_24282,-alive(bob),_24286) is not translated: 
