...
- **Dates**: `2026-03-01`; day arithmetic with `the date is a number days
  ...
  earlier); `is before or equal to`, `is after`. A period the text gives in
  MONTHS or years is counted in calendar months, never as days: "within six
  months before the order" is `a limit is 6 months after the other date and
  the date is before or equal to the limit` (six months after 28 August is
  28 February; 31 August + 6 months is 28 February). A period given in days
  ("the 91st day", "30 days") stays in days.
...
- **ICD-10 code tables** of the Policy Article are data, not prose:
  ...
  a table (docs/user/reference/language.md §17.3, `examples/regulatory/loaded_table.le` of LogicalEnglish2);
...
- **Run from the LogicalEnglish2 repository root**, never from the medicare directory:
  ```
  ./myswipl.sh -g "use_module(le_kbs), verify('examples/regulatory/medicare/<policy>_cases.le'), halt." 2>&1 | grep -v "untested_predicate\|Fix: add a query\|Position:" 
  ```
