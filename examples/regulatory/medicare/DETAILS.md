# Medicare DMEPOS coverage model: details

Step 4 of the plan in RulesRUs §8 (the authors' planning report, not published) —
§2.1 of that report is the plan, this
...
| `<policy>_cases.le` | Synthetic claims: a complete payable claim, one failure per criterion, the alternative routes, a documentation failure with its stage and flip, a Council-style unsupported finding. `pap_cases.le`, `pmd_cases.le`, `oxygen_cases.le` and `glucose_monitors_cases.le` also declare a reviewer's view, the *coverage desk* (why a claim is not payable, with the passages it fails; what documentation is missing; a flip that keeps the claim's item, code, beneficiary and dates; an approval and a denial letter). |
...
The directory is `examples/regulatory/medicare/` of the LogicalEnglish2
repository. Run a program's tests from the LogicalEnglish2 root:

```
./myswipl.sh -g "use_module(le_kbs), runTestsFor('examples/regulatory/medicare/pmd_cases.le', R), print_test_result(R), halt."
```

(the core example suite, `runTests`, runs them all; in the editor they are
`regulatory/medicare/<program>`, and their old names
`lpsPlus/medicare/<program>` still open them).
...
text in `sources/`; the editor's § badge opens it. Months are calendar months
(six months after 28 August is 28 February), days are days.
...
The editor's "facts from a document" path (docs/user/reference/language.md §17.9) run
