# Customs classification model: details

Step 1 of the plan in RulesRUs §8 (customs, §1 of that report — RulesRUs is
the authors' planning report, not published), scaled up:
...
The directory is `examples/regulatory/customs/` of the LogicalEnglish2
repository. Run one of the programs from the LogicalEnglish2 root, e.g.

```
./myswipl.sh -g "use_module(le_kbs), runTestsFor('examples/regulatory/customs/cbp_61.le', R), print_test_result(R), halt."
```

(the core example suite, `runTests`, runs them all; in the editor they are
`regulatory/customs/<program>`, and their old names `lpsPlus/customs/<program>`
still open them).
...
(docs/user/reference/language.md §15.5):
...
text (docs/user/reference/language.md §17.9). Review the rows, *Insert*, run `subheading`.
...
Two defects of the regulatory-decision constructs (docs/user/reference/language.md §17)
