...
2. add one `scenario <name> is:` per case (CASES material), with facts only
   from case-datum templates and embedded expectations
   (`<queryname> expects answers [...]` — no leading `query` keyword; numbers
   without thousands separators), plus at least one adversarial variant per
   major exception;
